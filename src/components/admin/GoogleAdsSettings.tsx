...
import { AlertCircle, CheckCircle2, ExternalLink, KeyRound, Link2, LogOut, RefreshCw, ShieldCheck, Target } from 'lucide-react';
...
export const GoogleAdsSettings: React.FC<GoogleAdsSettingsProps> = ({ onOpenCampaigns }) => {
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState<{ configured: boolean; apiVersion: string; connection: GoogleAdsConnection | null } | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (nextStatus.connection?.status === 'connected') {
        ...
        setStatus((previous) => previous ? { ...previous, connection: assets.connection } : previous);
      } else {
        setCustomers([]);
      }
    } catch (error: any) {
      const text = error?.message || 'Não foi possível carregar a integração Google Ads.';
      setLoadError(text);
  ...
  useEffect(() => {
    ...
    if (result === 'connected') setMessage({ type: 'success', text: 'Conta Google autorizada. Confirme a conta de anúncios abaixo.' });
    if (result === 'error') setMessage({ type: 'error', text: reason || 'A autorização Google não foi concluída.' });
  ...
  const connect = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error?.message || 'Não foi possível iniciar a conexão Google Ads.' });
      setBusy(false);
    }
  };
  ...
  const refreshAssets = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error?.message || 'Não foi possível atualizar as contas Google Ads.' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const saveCustomer = async () => {
    ...
    try {
      ...
      setStatus((previous) => previous ? { ...previous, connection: response.connection } : previous);
      setLoadError(null);
      setMessage({ type: 'success', text: 'Conta Google Ads salva. O módulo Campanhas já pode usar esta conta.' });
    } catch (error: any) {
      setMessage({ type: 'error', text: error?.message || 'Não foi possível salvar a conta Google Ads.' });
    } finally {
      setBusy(false);
    }
  };
  ...
  if (loading) return <div className="space-y-4"><AdminPageHeader icon={Target} title="Google Ads" /><AdminListSkeleton rows={5} /></div>;
