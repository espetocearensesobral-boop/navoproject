...
const MetricCard: React.FC<{
 ...
 helper?: string;
 tone?: string;
}> = ({ icon: Icon, label, value, helper, tone ="text-status-info"}) => (
...
export const GoogleAdsManagement: React.FC<GoogleAdsManagementProps> = ({
 onOpenSettings,
}) => {
 ...
 const [connection, setConnection] = useState<GoogleAdsConnection | null>(
 null,
 );
 ...
 const text =
 error?.message ||"Não foi possível carregar o módulo Google Ads.";
 setLoadError(text);
 ...
 const updateForm = <K extends keyof GoogleCampaignForm>(
 key: K,
 value: GoogleCampaignForm[K],
 ...
 ...form,
 name: form.name.trim(),
 headlines: form.headlines.map((item) => item.trim()).filter(Boolean),
 ...
 const response = await updateGoogleCampaignStatus(
 ...
 Boolean(connection.customerId);

 if (loading)
 return (
 <div className="space-y-4">
 <AdminPageHeader icon={Megaphone} title="Google Ads"/>
 ...
 <div>
 <h2 className="text-sm font-bold text-[var(--admin-text-main)]">
 Conecte o Google Ads para começar
 </h2>
 <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
 A conexão e os ajustes ficam em Configurações. O módulo só cria
 campanhas depois que uma conta cliente for escolhida.
 </p>
 </div>
 <button
 type="button"
 ...
 <div>
 <h2 className="text-sm font-bold text-[var(--admin-text-main)]">
 Finalize a seleção da conta
 </h2>
 <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
 Selecione uma conta cliente Google Ads em Configurações antes de
 criar ou sincronizar campanhas.
 </p>
 </div>
 <button
 type="button"
 ...
 icon={BarChart3}
 label="Conversões"
 value={formatCompactNumber(Math.round(totals.conversions))}
 helper="Conversões configuradas"
 tone="text-cyan-300"
 />
 </div>
 ...
 <div>
 <h2 className="text-sm font-bold text-[var(--admin-text-main)]">
 Suas campanhas Google
 </h2>
 <p className="mt-1 text-xs text-[var(--admin-text-muted)]">
 {CAMPAIGNS_DEMO_MODE
 ?"Os dados exibidos são ilustrativos e não representam uma conta Google Ads real."
 :"As métricas são sincronizadas com a conta Google Ads selecionada."}
 </p>
 </div>
 ...
 :"Crie uma campanha pausada para revisar os dados antes de ativar."
 :"Conecte o Google Ads e selecione a conta em Configurações."
 }
 ...
 <article
 key={campaign.id}
 className="rounded-[var(--admin-radius-md)] border border-[var(--admin-border)] bg-[var(--admin-bg)] p-3.5 transition-colors hover:border-status-info/30"
 >
 <div className="flex flex-wrap items-start justify-between gap-3">
 <div className="min-w-0">
 ...
 : `https://ads.google.com/aw/campaigns?ocid=${encodeURIComponent(connection?.customerId ||"")}`
 }
 target="_blank"
 rel="noreferrer"
 ...
 <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[var(--admin-border)] pt-3 sm:grid-cols-6">
 <div>
 ...
 {formatCompactNumber(Math.round(campaign.conversions))}
 </p>
 </div>
 <div>
 <p className="text-[10px] text-[var(--admin-text-muted)]">
 ...
 :"Será criada pausada para revisão no Google Ads."}
 </p>
 </div>
 <button
 type="button"
...
"keywords",
 ...
 <p className="max-w-xl text-[11px] leading-relaxed text-[var(--admin-text-muted)]">
 {CAMPAIGNS_DEMO_MODE
 ?"Nada será enviado para o Google. Esta ação cria apenas um registro local demonstrativo."
 :"A campanha, o grupo, as palavras-chave e o anúncio serão criados pausados. A ativação pode gerar cobrança na conta Google Ads e exige uma confirmação separada."}
 </p>
 ...
 : `A campanha “${statusTarget?.campaign.name}” será ativada no Google Ads. O orçamento diário informado poderá gerar cobrança.`
 : CAMPAIGNS_DEMO_MODE
 ? `A campanha “${statusTarget?.campaign.name}” será marcada como pausada somente nesta demonstração.`
 : `A campanha “${statusTarget?.campaign.name}” será pausada no Google Ads e deixará de veicular.`
 }
 confirmText={
 statusTarget?.status ==="ENABLED"
 ? CAMPAIGNS_DEMO_MODE
 ...
 variant={statusTarget?.status ==="ENABLED"?"primary":"danger"}
 icon={
 statusTarget?.status ==="ENABLED"? (
