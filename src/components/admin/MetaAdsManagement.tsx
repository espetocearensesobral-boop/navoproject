...
export const MetaAdsManagement: React.FC<MetaAdsManagementProps> = ({ onOpenSettings }) => {
  ...
  useEffect(() => {
    if (!formOpen) return;
    const scrollContainer = document.querySelector<HTMLElement>('.admin-shell main');
    if (!scrollContainer) return;
    const previousOverflowY = scrollContainer.style.overflowY;
    scrollContainer.style.overflowY = 'hidden';
    return () => { scrollContainer.style.overflowY = previousOverflowY; };
  }, [formOpen]);
