...
const statusClass: Record<string, string> = {
 ...
 WITH_ISSUES:"bg-status-error/10 text-status-error",
};
...
const MetricCard: React.FC<{
 ...
 <p className={`mt-2 text-xl font-bold tracking-tight ${tone}`}>
 {value}
 </p>
 </div>
 <div
...
export const MetaAdsManagement: React.FC<MetaAdsManagementProps> = ({
 onOpenSettings,
}) => {
 ...
 const text =
 error?.message ||"Não foi possível carregar o módulo Campanhas.";
 setLoadError(text);
 ...
 const nextTotals = response.campaigns.reduce(
 (acc, campaign) => ({
 ...
 const handleFormChange = <K extends keyof MetaCampaignForm>(
 key: K,
 value: MetaCampaignForm[K],
 ...
 ...form,
 name: form.name.trim(),
 dailyBudgetCents: Math.round(Number(form.dailyBudgetCents)),
 },
 campaigns.length,
 );
 ...
 campaign.id === statusTarget.campaign.id
 ? {
 ...campaign,
 ...
 ?"Campanha ativada apenas na demonstração."
 :"Campanha pausada apenas na demonstração.",
 });
 return;
 }
 const response = await updateMetaCampaignStatus(
 statusTarget.campaign.id,
 statusTarget.status,
 );
 setCampaigns((previous) =>
 previous.map((campaign) =>
 campaign.id === response.campaign.id ? response.campaign : campaign,
 ),
 );
 setMessage({
 type:"success",
 ...
 Boolean(connection.adAccountId && connection.pageId);

 if (loading)
 return (
 <div className="space-y-4">
 <AdminPageHeader icon={Megaphone} title="Campanhas"/>
 ...
 {message && (
 <div
 className={`flex items-start gap-2 rounded-[var(--admin-radius-md)] border px-3 py-2.5 text-sm ${message.type ==="success"?"border-status-success/30 bg-status-success/10 text-status-success":"border-status-error/30 bg-status-error/10 text-status-error"}`}
 role="status"
 >
 ...
 {loadError && (
 <section className="rounded-[var(--admin-radius-lg)] border border-status-warning/30 bg-status-warning/10 p-4">
 ...
 onClick={() => void load()}
 disabled={loading}
 className="inline-flex min-h-9 shrink-0 items-center gap-1.5 rounded-[var(--admin-radius-sm)] border border-status-warning/30 px-3 text-xs font-bold text-status-warning hover:bg-status-warning/10 disabled:opacity-50"
 >
 <RefreshCw
 ...
 <div>
 <h2 className="text-sm font-bold text-[var(--admin-text-main)]">
 Conecte o Meta Ads para começar
 </h2>
 <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
 A conexão e os ajustes ficam em Configurações. O módulo só cria
 campanhas depois que a conta e a Página forem escolhidas.
 </p>
 </div>
 <button
 type="button"
 ...
 <div>
 <h2 className="text-sm font-bold text-[var(--admin-text-main)]">
 Finalize a seleção de ativos
 </h2>
 <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
 Selecione uma conta de anúncios e uma Página em Configurações
 antes de criar ou sincronizar campanhas.
 </p>
 </div>
 <button
 type="button"
 onClick={onOpenSettings}
 className="min-h-9 rounded-[var(--admin-radius-sm)] border border-status-warning/30 px-3 text-xs font-bold text-status-warning hover:bg-status-warning/10"
 >
 ...
 icon={BarChart3}
 label="Investido"
 value={formatCents(totals.spendCents, connection?.currency ||"BRL")}
 helper={
 CAMPAIGNS_DEMO_MODE
 ...
 icon={Users}
 label="Leads"
 value={formatCompactNumber(totals.leads)}
 helper={
 CAMPAIGNS_DEMO_MODE
 ?"Conversões demonstrativas"
 :"Ações de lead retornadas"
 }
 ...
 {CAMPAIGNS_DEMO_MODE
 ?"Os dados exibidos são ilustrativos e não representam uma conta Meta real."
 :"As métricas exibidas são sincronizadas com a conta Meta selecionada."}
 </p>
 </div>
 ...
 title={
 CAMPAIGNS_DEMO_MODE
 ?"Nenhuma campanha demonstrativa"
 :"Nenhuma campanha sincronizada"
 }
 description={
 canCreate
 ? CAMPAIGNS_DEMO_MODE
 ?"Crie uma campanha local pausada para visualizar o fluxo."
 :"Crie uma campanha pausada para revisar o anúncio antes de ativar."
 :"Conecte a Meta e selecione os ativos em Configurações."
 }
 actionLabel={canCreate ?"Criar campanha": undefined}
 ...
 <article
 key={campaign.id}
 className="rounded-[var(--admin-radius-md)] border border-[var(--admin-border)] bg-[var(--admin-bg)] p-3.5 transition-colors hover:border-[var(--admin-accent)]/40"
 >
 <div className="flex flex-wrap items-start justify-between gap-3">
 <div className="min-w-0">
 <div className="flex flex-wrap items-center gap-2">
 <h3 className="truncate text-sm font-bold text-[var(--admin-text-main)]">
 {campaign.name}
 </h3>
 <span
 ...
 <p className="mt-1 text-xs text-[var(--admin-text-muted)]">
 {campaign.locationLabel ||"Localização não informada"}{""}
 ·{""}
 {formatCents(
 campaign.dailyBudgetCents,
 connection?.currency ||"BRL",
 )}
 /dia
 </p>
 </div>
 ...
 : `https://www.facebook.com/adsmanager/manage/campaigns?act=${encodeURIComponent(connection?.adAccountId?.replace(/^act_/,"") ||"")}`
 }
 target="_blank"
 rel="noreferrer"
 ...
 >
 {CAMPAIGNS_DEMO_MODE ?"Referência Meta":"Meta"}{""}
 ...
 className="inline-flex h-8 items-center gap-1 rounded-[var(--admin-radius-sm)] border border-status-warning/30 px-2.5 text-[11px] font-bold text-status-warning hover:bg-status-warning/10"
 >
 ...
 type="button"
 onClick={() =>
 setStatusTarget({ campaign, status:"ACTIVE"})
 }
 disabled={!canCreate}
 ...
 <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[var(--admin-border)] pt-3 sm:grid-cols-5">
 <div>
 <p className="text-[10px] text-[var(--admin-text-muted)]">
 Gasto
 </p>
 <p className="mt-1 text-xs font-bold finance-negative">
 {formatCents(
 campaign.spendCents,
 connection?.currency ||"BRL",
 )}
 ...
 <p className="mt-1 text-xs font-bold text-status-success">
 {formatCompactNumber(campaign.leads)}
 </p>
 </div>
 <div>
 <p className="text-[10px] text-[var(--admin-text-muted)]">
 ...
 <p className="mt-1 text-xs font-bold text-status-info">
 ...
 <p className="mt-1 text-xs font-bold text-[var(--admin-text-main)]">
 ...
 <p className="mt-1 text-xs font-bold text-[var(--admin-text-main)]">
 {campaign.startDate ||"Agora"}
 {campaign.endDate ? ` → ${campaign.endDate}` :""}
 </p>
 </div>
 </div>
 {campaign.errorMessage && (
 <p className="mt-3 rounded-[var(--admin-radius-sm)] bg-status-error/10 px-3 py-2 text-xs text-status-error">
 {campaign.errorMessage}
 </p>
 )}
 </article>
 ...
 createPortal(
 <div
 className="fixed inset-0 z-[180] flex h-[100dvh] w-full items-center justify-center overflow-hidden overscroll-none bg-black/70 p-0 touch-none sm:p-4"
 role="dialog"
 aria-modal="true"
 aria-labelledby="campaign-form-title"
 >
 <div className="flex h-full max-h-full w-full max-w-2xl min-w-0 flex-col overflow-hidden rounded-none border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-2xl sm:h-[min(720px,calc(100dvh-2rem))] sm:max-h-[calc(100dvh-2rem)] sm:rounded-[var(--admin-radius-xl)]">
 ...
 className="text-base font-bold text-[var(--admin-text-main)]"
 >
 Nova campanha
 </h2>
 <p className="mt-1 text-xs text-[var(--admin-text-muted)]">
 {CAMPAIGNS_DEMO_MODE
 ?"A campanha será criada pausada apenas nesta demonstração."
 :"A campanha será criada pausada para revisão."}
 </p>
 </div>
 <button
 type="button"
 ...
 <form
 onSubmit={create}
 className="min-h-0 min-w-0 flex-1 touch-pan-y overflow-x-hidden overflow-y-auto overscroll-contain space-y-4 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-5"
 >
 ...
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 maxLength={120}
 required
 />
 </label>
 ...
 handleFormChange(
"objective",
 event.target.value as"OUTCOME_TRAFFIC",
 )
 }
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 >
...
"dailyBudgetCents",
 ...
 <MapPin className="h-3.5 w-3.5"/> Localização exibida
 </span>
 <input
 value={form.locationLabel}
 onChange={(event) =>
 handleFormChange("locationLabel", event.target.value)
 }
 placeholder="Sobral e região"
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 required
 />
 </label>
 <label className="text-xs font-semibold text-[var(--admin-text-muted)] sm:col-span-2">
 Chave de cidade Meta (opcional)
 <input
 value={form.locationKey}
 onChange={(event) =>
 handleFormChange("locationKey", event.target.value)
 }
 placeholder="Informe a chave se quiser segmentar uma cidade específica"
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 />
 </label>
 <label className="text-xs font-semibold text-[var(--admin-text-muted)] sm:col-span-2">
 ...
 onChange={(event) =>
 handleFormChange("destinationUrl", event.target.value)
 }
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 required
 />
 ...
 value={form.adText}
 onChange={(event) =>
 handleFormChange("adText", event.target.value)
 }
 rows={3}
 maxLength={500}
 className="mt-1.5 w-full resize-y rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 py-2 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 required
 />
 </label>
 <label className="text-xs font-semibold text-[var(--admin-text-muted)]">
 ...
 value={form.headline}
 onChange={(event) =>
 handleFormChange("headline", event.target.value)
 }
 maxLength={80}
 className="mt-1.5 h-10 w-full rounded-[var(--admin-radius-sm)] border border-[var(--admin-border)] bg-[var(--admin-bg)] px-3 text-sm text-[var(--admin-text-main)] outline-none focus:border-[var(--admin-accent)]"
 required
 ...
 <label className="text-xs font-semibold text-[var(--admin-text-muted)]">
 <span className="flex items-center gap-1">
 <ImagePlus className="h-3.5 w-3.5"/> URL pública da
 imagem
 </span>
 <input
 type="url"
 ...
 <p className="max-w-md text-[11px] leading-relaxed text-[var(--admin-text-muted)]">
 {CAMPAIGNS_DEMO_MODE
 ?"Nada será enviado para a Meta. Esta ação cria apenas um registro local demonstrativo."
 :"A Meta poderá revisar o anúncio. O Navo não ativa a campanha sem uma ação explícita no botão Ativar."}
 </p>
 ...
 statusTarget?.status ==="ACTIVE"
 ? CAMPAIGNS_DEMO_MODE
 ? `A campanha “${statusTarget?.campaign.name}” será marcada como ativa somente nesta demonstração. Nenhum orçamento será cobrado.`
 : `A campanha “${statusTarget?.campaign.name}” será ativada na Meta. O orçamento diário informado poderá ser cobrado pela conta de anúncios.`
 : CAMPAIGNS_DEMO_MODE
 ? `A campanha “${statusTarget?.campaign.name}” será marcada como pausada somente nesta demonstração.`
 : `A campanha “${statusTarget?.campaign.name}” será pausada na Meta e deixará de veicular.`
 }
 confirmText={
 statusTarget?.status ==="ACTIVE"
 ? CAMPAIGNS_DEMO_MODE
