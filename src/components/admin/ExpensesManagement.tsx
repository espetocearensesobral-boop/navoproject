    ...
    <div className="space-y-4 animate-fade-in text-content-base min-w-0">
      ...
      <div className="bg-surface-card border border-border-subtle rounded-xl overflow-hidden">
        <div className="p-4 sm:p-5 border-b border-border-subtle flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div><h2 className="text-base font-serif font-bold text-content-base">Livro de saídas</h2><p className="mt-0.5 text-sm text-content-muted">Cada despesa registrada aqui entra imediatamente no Extrato real e nos Relatórios.</p></div>
          <div className="relative w-full sm:w-72"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-content-muted" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar descrição ou categoria" className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle pl-9 pr-3 text-sm text-content-base placeholder:text-content-muted focus:outline-none focus:border-gold-base" /></div>
        </div>

        {loading ? <div className="py-16 text-center text-sm text-content-muted"><RefreshCw className="w-4 h-4 inline animate-spin mr-2" />Carregando saídas persistidas…</div> : visibleExpenses.length === 0 ? <div className="py-16 px-5 text-center"><ArrowDownRight className="w-10 h-10 mx-auto text-content-muted/50" /><h3 className="mt-3 text-base font-bold text-content-base">Nenhuma saída registrada</h3><p className="mt-1 text-sm text-content-muted">Use “Nova saída” para lançar uma despesa real no livro-caixa.</p></div> : <div className="divide-y divide-border-subtle">{visibleExpenses.map((expense) => <article key={expense.id} className="p-4 sm:px-5 flex items-start gap-3"><span className="w-10 h-10 shrink-0 rounded-xl bg-status-error/10 text-status-error flex items-center justify-center"><ArrowDownRight className="w-5 h-5" /></span><div className="flex-1 min-w-0"><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><h3 className="text-sm font-bold text-content-base truncate">{expense.description}</h3><span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-surface-base border border-border-subtle text-content-muted">{expense.category}</span></div><p className="mt-1 text-xs text-content-muted">{expense.date.split('-').reverse().join('/')} · {paymentMethods.find((method) => method.id === expense.paymentMethod)?.label || expense.paymentMethod}</p>{expense.notes && <p className="mt-1 text-xs text-content-muted truncate">{expense.notes}</p>}</div><div className="text-right shrink-0"><p className="text-sm font-mono font-bold finance-negative">- {money(Number(expense.amount))}</p><div className="mt-2 flex justify-end gap-1"><button type="button" onClick={() => openEdit(expense)} className="w-8 h-8 rounded-lg border border-border-subtle text-content-muted hover:text-content-base flex items-center justify-center" aria-label={`Editar ${expense.description}`}><Edit3 className="w-3.5 h-3.5" /></button><button type="button" onClick={() => handleDelete(expense)} className="w-8 h-8 rounded-lg border border-status-error/25 text-status-error flex items-center justify-center" aria-label={`Excluir ${expense.description}`}><Trash2 className="w-3.5 h-3.5" /></button></div></div></article>)}</div>}
      </div>

      {isModalOpen && <div className="fixed inset-0 z-[90] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-5"><div className="admin-modal w-full max-w-xl rounded-t-2xl sm:rounded-2xl bg-surface-card border border-border-subtle max-h-[94dvh] overflow-y-auto"><div className="sticky top-0 z-10 p-5 sm:p-6 bg-surface-card border-b border-border-subtle flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-gold-base">Livro-caixa</p><h2 className="mt-1 text-lg font-serif font-bold text-content-base">{editingExpense ? 'Editar saída' : 'Nova saída'}</h2></div><button type="button" onClick={closeModal} className="w-10 h-10 rounded-xl text-content-muted hover:text-content-base hover:bg-surface-base flex items-center justify-center" aria-label="Fechar formulário"><X className="w-5 h-5" /></button></div><form onSubmit={handleSave} onKeyDown={handleEnterAsTab} className="p-5 sm:p-6 space-y-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><label className="sm:col-span-2"><span className="text-sm font-bold text-content-base block mb-1.5">Descrição *</span><input autoFocus value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Ex.: Compra de produtos para revenda" className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle px-3 text-sm text-content-base placeholder:text-content-muted focus:outline-none focus:border-gold-base" /></label><label><span className="text-sm font-bold text-content-base block mb-1.5">Categoria *</span><select value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))} className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle px-3 text-sm text-content-base focus:outline-none focus:border-gold-base">{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label><span className="text-sm font-bold text-content-base block mb-1.5">Data *</span><input type="date" value={form.date} onChange={(event) => setForm((current) => ({ ...current, date: event.target.value }))} className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle px-3 text-sm text-content-base focus:outline-none focus:border-gold-base" /></label><label><span className="text-sm font-bold text-content-base block mb-1.5">Valor *</span><input type="number" min="0.01" step="0.01" value={form.amount || ''} onChange={(event) => setForm((current) => ({ ...current, amount: Number(event.target.value) }))} placeholder="0,00" className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle px-3 text-sm font-mono font-bold finance-negative focus:outline-none focus:border-gold-base" /></label><label><span className="text-sm font-bold text-content-base block mb-1.5">Forma de pagamento</span><select value={form.paymentMethod} onChange={(event) => setForm((current) => ({ ...current, paymentMethod: event.target.value }))} className="w-full h-11 rounded-xl bg-surface-base border border-border-subtle px-3 text-sm text-content-base focus:outline-none focus:border-gold-base">{paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.label}</option>)}</select></label><label className="sm:col-span-2"><span className="text-sm font-bold text-content-base block mb-1.5">Observações</span><textarea rows={3} value={form.notes || ''} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Informação complementar do lançamento" className="w-full rounded-xl bg-surface-base border border-border-subtle px-3 py-3 text-sm text-content-base placeholder:text-content-muted resize-none focus:outline-none focus:border-gold-base" /></label></div><div className="pt-4 border-t border-border-subtle flex flex-col-reverse sm:flex-row sm:justify-end gap-2"><button type="button" onClick={closeModal} className="h-11 px-5 rounded-xl border border-border-subtle text-content-muted hover:text-content-base text-sm font-bold">Cancelar</button><button type="submit" disabled={saving} className="h-11 px-5 rounded-xl bg-gold-base text-surface-base text-sm font-bold disabled:opacity-50">{saving ? 'Salvando…' : 'Salvar saída'}</button></div></form></div></div>}
    </div>
  );
};
