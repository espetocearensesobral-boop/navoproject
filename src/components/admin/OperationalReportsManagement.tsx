...
const QueueSnapshot: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={Users} title="Fila e operação" description={`${report.summary.currentQueue} cliente${report.summary.currentQueue === 1 ? '' : 's'} na operação neste momento.`} /><div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">{report.queueSummary.filter((item) => ['waiting', 'in_chair', 'completed', 'abandoned'].includes(item.status)).map((item) => <div key={item.status} className="p-3 rounded-xl bg-surface-base border border-border-subtle"><p className="text-xs text-content-muted admin-safe-wrap">{queueLabels[item.status] || item.status}</p><p className="mt-1 text-xl font-mono font-bold text-content-base">{item.count}</p></div>)}</div></section>;

const TodaySchedule: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={CalendarDays} title="Próximos horários de hoje" description="Agenda operacional aberta para o dia atual." />{report.upcomingAppointments.length === 0 ? <EmptyState text="Nenhum próximo atendimento ativo hoje." /> : <div className="mt-4 divide-y divide-border-subtle">{report.upcomingAppointments.map((appointment) => <div key={appointment.id} className="py-3 first:pt-0 last:pb-0 flex items-center gap-3"><span className="w-12 text-center text-sm font-mono font-bold text-gold-base">{appointment.timeSlot}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-content-base admin-clamp-2">{appointment.clientName}</p><p className="mt-0.5 text-xs text-content-muted admin-safe-wrap">{appointment.serviceTitle} · {appointment.professionalName}</p></div><span className="text-xs font-bold px-2 py-1 rounded-full bg-surface-base text-content-muted shrink-0">{statusLabels[appointment.status] || appointment.status}</span></div>)}</div>}</section>;
