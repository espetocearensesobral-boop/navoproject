    ...
    <div className="space-y-4 animate-fade-in text-content-base min-w-0">
      <AdminPageHeader
        ...
        stats={report ? [
          ...
          { label: 'pico', value: report.peakHour?.label || '—', tone: 'warning' },
        ] : undefined}
...
const WeeklyMovement: React.FC<{ report: OperationalReportData; maxWeekly: number }> = ({ report, maxWeekly }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={BarChart3} title="Movimento semanal" description="Agendamentos por dia; concluídos em destaque." /><div className="mt-6 h-52 flex items-end gap-2 sm:gap-3">{report.weeklyMovement.map((day) => <div key={day.weekday} className="flex-1 min-w-0 h-full flex flex-col justify-end items-center gap-2"><div className="w-full h-[calc(100%-1.75rem)] flex items-end justify-center gap-1"><span title={`${day.appointments} agendamentos`} className="w-3 sm:w-5 rounded-t-md bg-gold-base/30 relative" style={{ height: `${Math.max(day.appointments ? 7 : 0, (day.appointments / maxWeekly) * 100)}%` }}><i className="absolute bottom-0 left-0 right-0 rounded-t-md bg-gold-base" style={{ height: `${day.appointments ? Math.max(8, (day.completed / day.appointments) * 100) : 0}%` }} /></span></div><span className="text-xs font-bold text-content-muted">{day.label}</span></div>)}</div><div className="mt-4 flex flex-wrap gap-4 text-xs text-content-muted"><span className="flex items-center gap-1.5"><i className="w-2.5 h-2.5 rounded-sm bg-gold-base" />Concluídos</span><span className="flex items-center gap-1.5"><i className="w-2.5 h-2.5 rounded-sm bg-gold-base/30" />Total agendado</span></div></section>;

const DailyMovement: React.FC<{ report: OperationalReportData }> = ({ report }) => { const max = Math.max(1, ...report.dailyMovement.map((day) => day.appointments)); return <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={CalendarDays} title="Evolução diária" description="Volume no período." /><div className="mt-5 space-y-2.5 max-h-64 overflow-y-auto pr-1">{report.dailyMovement.filter((day) => day.appointments > 0).map((day) => <div key={day.date} className="flex items-center gap-3"><span className="w-12 text-xs font-mono text-content-muted">{day.label}</span><div className="flex-1 h-2 rounded-full bg-surface-base overflow-hidden"><span className="block h-full rounded-full bg-gold-base" style={{ width: `${(day.appointments / max) * 100}%` }} /></div><span className="w-8 text-right text-xs font-bold text-content-base">{day.appointments}</span><span className="w-20 text-right text-xs finance-positive">{money(day.revenue)}</span></div>)}{report.dailyMovement.every((day) => day.appointments === 0) && <EmptyState text="Nenhum agendamento no período." />}</div></section>; };

const PeakHours: React.FC<{ report: OperationalReportData; maxHour: number }> = ({ report, maxHour }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={Clock3} title="Horários de pico" description={report.peakHour ? `Maior movimento concentrado às ${report.peakHour.label}.` : 'Sem dados suficientes para identificar um pico.'} />{report.topHours.length === 0 ? <EmptyState text="Nenhum horário registrado no período." /> : <div className="mt-5 space-y-3">{report.topHours.map((hour, index) => <div key={hour.hour} className="flex items-center gap-3"><span className="w-6 text-xs font-bold text-gold-base">#{index + 1}</span><span className="w-10 text-sm font-mono font-bold text-content-base">{hour.label}</span><div className="flex-1 h-2 rounded-full bg-surface-base overflow-hidden"><span className="block h-full rounded-full bg-status-warning" style={{ width: `${(hour.count / maxHour) * 100}%` }} /></div><span className="w-20 text-right text-xs text-content-muted">{hour.count} agend.</span></div>)}</div>}</section>;

const TopServices: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={Scissors} title="Top 5 serviços" description="Serviços mais movimentados no período." />{report.topServices.length === 0 ? <EmptyState text="Nenhum serviço encontrado no período." /> : <div className="mt-4 space-y-2.5">{report.topServices.map((service, index) => <div key={service.serviceTitle} className="p-3 rounded-xl bg-surface-base border border-border-subtle flex items-center gap-3"><span className="w-8 h-8 rounded-lg bg-gold-base/10 text-gold-base text-xs font-bold flex items-center justify-center shrink-0">#{index + 1}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-content-base admin-clamp-2">{service.serviceTitle}</p><p className="mt-0.5 text-xs text-content-muted">{service.count} agendamento{service.count === 1 ? '' : 's'} · {service.completedCount} concluído{service.completedCount === 1 ? '' : 's'}</p></div><div className="text-right shrink-0"><p className="text-sm font-mono font-bold finance-positive">{money(service.revenue)}</p><p className="text-xs text-content-muted">recebido</p></div></div>)}</div>}</section>;

const ProfessionalsRanking: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={UserRound} title="Performance da equipe" description="Atendimentos concluídos e receita por profissional." />{report.topProfessionals.length === 0 ? <EmptyState text="Nenhum profissional com atividade no período." /> : <div className="mt-4 space-y-2.5">{report.topProfessionals.map((professional, index) => <div key={professional.professionalName} className="p-3 rounded-xl bg-surface-base border border-border-subtle flex items-center gap-3"><span className="w-8 h-8 rounded-lg bg-gold-base/10 text-gold-base text-xs font-bold flex items-center justify-center shrink-0">#{index + 1}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-content-base admin-clamp-2">{professional.professionalName}</p><p className="mt-0.5 text-xs text-content-muted">{professional.completed} concluído{professional.completed === 1 ? '' : 's'} de {professional.appointments} agenda{professional.appointments === 1 ? '' : 's'}</p></div><p className="text-sm font-mono font-bold finance-positive shrink-0">{money(professional.revenue)}</p></div>)}</div>}</section>;

const QueueSnapshot: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={Users} title="Fila e operação" description={`${report.summary.currentQueue} cliente${report.summary.currentQueue === 1 ? '' : 's'} na operação agora.`} /><div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">{report.queueSummary.filter((item) => ['waiting', 'in_chair', 'completed', 'abandoned'].includes(item.status)).map((item) => <div key={item.status} className="p-3 rounded-xl bg-surface-base border border-border-subtle"><p className="text-xs text-content-muted admin-safe-wrap">{queueLabels[item.status] || item.status}</p><p className="mt-1 text-xl font-mono font-bold text-content-base">{item.count}</p></div>)}</div></section>;

const TodaySchedule: React.FC<{ report: OperationalReportData }> = ({ report }) => <section className="bg-surface-card border border-border-subtle rounded-xl p-4 sm:p-5"><SectionTitle icon={CalendarDays} title="Próximos hoje" description="Agenda de hoje." />{report.upcomingAppointments.length === 0 ? <EmptyState text="Nenhum atendimento ativo hoje." /> : <div className="mt-4 divide-y divide-border-subtle">{report.upcomingAppointments.map((appointment) => <div key={appointment.id} className="py-3 first:pt-0 last:pb-0 flex items-center gap-3"><span className="w-12 text-center text-sm font-mono font-bold text-gold-base">{appointment.timeSlot}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-content-base admin-clamp-2">{appointment.clientName}</p><p className="mt-0.5 text-xs text-content-muted admin-safe-wrap">{appointment.serviceTitle} · {appointment.professionalName}</p></div><span className="text-xs font-bold px-2 py-1 rounded-full bg-surface-base text-content-muted shrink-0">{statusLabels[appointment.status] || appointment.status}</span></div>)}</div>}</section>;
