...
export const ReportsManagement: React.FC = () => {
 ...
 setError(null);
 try {
 const data = await fetchFinancialReportFromSupabase(selectedPeriod, {
 strict: true,
 });
 setReport(data);
 } catch (requestError) {
 ...
 .then((settings) => setRefreshSeconds(settings.reportsRefreshSeconds))
 .catch(() => {});
 }, []);
 ...
 value: `${report.summary.incomeCount} entradas`,
 tone:"neutral",
 },
 {
 label:"pendente",
 value: money(report.summary.pendingAmount),
 tone: report.summary.pendingAmount > 0 ?"warning":"muted",
 },
 ]
 : undefined
 }
 ...
 detail={`${summary.incomeCount} lançamento${summary.incomeCount === 1 ?"":"s"} confirmado${summary.incomeCount === 1 ?"":"s"}`}
 icon={ArrowUpRight}
 tone="positive"
 />
 <MetricCard
 ...
 detail={`${summary.expenseCount} despesa${summary.expenseCount === 1 ?"":"s"} registrada${summary.expenseCount === 1 ?"":"s"}`}
 icon={ArrowDownRight}
 tone="negative"
 />
 <MetricCard
 ...
 detail={`${summary.pendingCount} recebimento${summary.pendingCount === 1 ?"":"s"} pendente${summary.pendingCount === 1 ?"":"s"}`}
 icon={WalletCards}
 ...
 <CalendarRange className="w-5 h-5 text-[var(--admin-accent)] shrink-0"/>
 </div>
 {report.dailyCashFlow.length === 0 ? (
 <EmptyState text="Não há lançamentos confirmados neste período."/>
 ) : (
 <div className="mt-6 h-52 flex items-end gap-2 sm:gap-3">
 {report.dailyCashFlow.map((day) => (
 <div
 key={day.date}
 className="flex-1 min-w-0 h-full flex flex-col justify-end items-center gap-1.5 group"
 >
 ...
 <span className="text-xs font-mono text-[var(--admin-text-muted)] whitespace-nowrap">
 ...
 value={money(summary.serviceRevenue)}
 tone="positive"
 />
 <InfoRow
 label="Outras entradas"
 value={money(summary.otherIncome)}
 tone="positive"
 />
 <InfoRow
 ...
 tone="positive"
 />
 <div className="pt-3 mt-3 border-t border-[var(--admin-border)]">
 <InfoRow
 ...
 description="Ranking por pagamentos confirmados."
 />
 {report.services.length === 0 ? (
 <EmptyState text="Nenhum recebimento de serviço confirmado no período."/>
 ) : (
 ...
 subtitle={`${service.count} atendimento${service.count === 1 ?"":"s"} · ticket ${money(service.averageTicket)}`}
 value={money(service.revenue)}
 percentage={
 summary.serviceRevenue
 ? (service.revenue / summary.serviceRevenue) * 100
 : 0
 }
 />
 ))}
 ...
 description="Distribuição dos recebimentos."
 />
 {report.paymentMethods.length === 0 ? (
 <EmptyState text="Nenhuma forma de pagamento registrada no período."/>
 ) : (
 <div className="mt-4 space-y-3">
 ...
 key={method.method}
 position={null}
 title={paymentLabel[method.method] || method.method}
 subtitle={`${method.count} pagamento${method.count === 1 ?"":"s"}`}
 value={money(method.total)}
 percentage={
 summary.serviceRevenue
 ? (method.total / summary.serviceRevenue) * 100
 : 0
 }
 />
 ))}
 ...
 description="Despesas do extrato real."
 />
 {report.expenseCategories.length === 0 ? (
 <EmptyState text="Nenhuma saída registrada no período."/>
 ) : (
 <div className="mt-4 space-y-3">
 {report.expenseCategories.map((expense, index) => (
 <RankRow
 key={expense.category}
 position={index + 1}
 title={expense.category}
 subtitle={`${expense.count} lançamento${expense.count === 1 ?"":"s"}`}
 ...
 <p
 className={`mt-2 text-2xl font-mono font-bold ${summary.netResult >= 0 ?"finance-positive":"finance-negative"}`}
 >
 {money(summary.netResult)}
 </p>
 <p className="mt-2 text-sm text-[var(--admin-text-muted)]">
 Entradas confirmadas de {money(summary.totalIncome)} menos
 despesas de {money(summary.totalExpenses)}.
 </p>
 </div>
 <div className="mt-4 text-sm text-[var(--admin-text-muted)] leading-relaxed">
 As saídas são incluídas no momento em que são salvas na aba{""}
 <strong className="text-[var(--admin-text-main)]">
 Saídas
 </strong>
 ; para corrigir um lançamento, edite-o ou remova-o nessa
 mesma aba, e o Extrato e os Relatórios serão atualizados.
 </div>
 </section>
 </div>
 )}
 ...
 description={`${summary.returningClientCount} cliente${summary.returningClientCount === 1 ?"":"s"} recorrente${summary.returningClientCount === 1 ?"":"s"} de ${summary.clientCount} no período · retenção de ${summary.retentionRate.toFixed(1)}%.`}
 />
 {report.clients.length === 0 ? (
 <EmptyState text="Nenhum cliente com recebimento confirmado no período."/>
 ) : (
 ...
 {client.visits} visita
 {client.visits === 1 ?"":"s"} recebida
 {client.visits === 1 ?"":"s"}
 {client.clientPhone
 ? ` · ${client.clientPhone}`
 :""}
 ...
 description="Receita e comissão estimada."
 />
 {report.professionals.length === 0 ? (
 <EmptyState text="Nenhum recebimento associado a profissional no período."/>
 ) : (
 <div className="mt-4 space-y-3">
 ...
 <p className="mt-0.5 text-xs text-[var(--admin-text-muted)]">
 {professional.servicesCount} serviço
 {professional.servicesCount === 1 ?"":"s"} ·
 comissão{""}
 {professional.commissionRate.toFixed(1)}%
 </p>
 </div>
 <p className="text-sm font-mono font-bold finance-positive shrink-0">
 ...
 <div className="mt-3 pt-3 border-t border-[var(--admin-border)] flex justify-between text-xs">
 <span className="text-[var(--admin-text-muted)]">
 Comissão estimada
 </span>
 <strong className="finance-negative">
 {money(professional.commissionAmount)}
 </strong>
 </div>
 </div>
 ))}
...
const MetricCard: React.FC<{
 ...
 : tone ==="warning"
 ?"text-amber-500 bg-status-warning/10"
 ...
 <p className="admin-copy-description text-xs text-[var(--admin-text-muted)] admin-safe-wrap">
 {detail}
 </p>
 </div>
 );
...
const RankRow: React.FC<{
 ...
 <strong
 className={`text-sm font-mono shrink-0 ${valueTone ==="negative"?"finance-negative":"finance-positive"}`}
 >
 {value}
 </strong>
 </div>
