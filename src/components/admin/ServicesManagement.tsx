                  ...
                  {isExpanded && (
                    <div className="border-t border-border-subtle bg-surface-base/35 p-3 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button type="button" onClick={() => handleToggleCombo(service)} title={service.is_combo ? 'Desativar Combo VIP' : 'Ativar Combo VIP'} aria-label={service.is_combo ? 'Desativar Combo VIP' : 'Ativar Combo VIP'} className={`min-h-10 min-w-10 px-2 sm:px-3 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${service.is_combo ? 'border-status-success/30 text-status-success' : 'border-border-subtle text-content-muted'}`}><Flame className="w-4 h-4" /><span className="hidden sm:inline">{service.is_combo ? 'Combo ativo' : 'Combo VIP'}</span></button>
                        <button type="button" onClick={() => handleTogglePopular(service)} title={service.popular ? 'Remover destaque' : 'Marcar como destaque'} aria-label={service.popular ? 'Remover destaque' : 'Marcar como destaque'} className={`min-h-10 min-w-10 px-2 sm:px-3 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${service.popular ? 'border-gold-base/40 text-gold-base' : 'border-border-subtle text-content-muted'}`}><Star className="w-4 h-4" /><span className="hidden sm:inline">{service.popular ? 'Destaque ativo' : 'Destaque'}</span></button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button type="button" onClick={() => handleDuplicate(service)} title="Duplicar serviço" aria-label="Duplicar serviço" className="min-h-10 min-w-10 px-2 sm:px-3 rounded-xl border border-border-subtle text-content-muted text-sm font-semibold flex items-center justify-center gap-1.5"><Copy className="w-4 h-4" /><span className="hidden sm:inline">Duplicar</span></button>
                        <button type="button" onClick={() => handleOpenEdit(service)} title="Editar serviço" aria-label="Editar serviço" className="min-h-10 min-w-10 px-2 sm:px-3 rounded-xl bg-gold-base text-surface-base text-sm font-bold flex items-center justify-center gap-1.5"><Edit2 className="w-4 h-4" /><span className="hidden sm:inline">Editar</span></button>
                        <button type="button" onClick={() => handleDelete(service.id)} title="Excluir serviço" aria-label="Excluir serviço" className="min-h-10 min-w-10 px-2 sm:px-3 rounded-xl border border-status-error/25 text-status-error text-sm font-semibold flex items-center justify-center gap-1.5"><Trash2 className="w-4 h-4" /><span className="hidden sm:inline">Excluir</span></button>
                      </div>
                    </div>
                  )}
                </article>
