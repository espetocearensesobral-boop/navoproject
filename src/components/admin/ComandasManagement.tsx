...
export const ComandasManagement: React.FC = () => {
        ...
        items: [
          {
            id: "s2",
            type: "service",
            title: "Barba Imperial + Toalha Quente",
            price: 50,
            quantity: 1,
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "pix" | "credit_card" | "debit_card" | "cash" | "split"
  >("pix");
  const [cashGiven, setCashGiven] = useState<string>("");
  const [discountInput, setDiscountInput] = useState<number>(0);
  const [tipInput, setTipInput] = useState<number>(0);
  const [receiptModalComanda, setReceiptModalComanda] =
  ...
  const closedComandas = comandas.filter(
    (c) =>
      c.status === "closed" &&
      (c.clientName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        c.code.toLowerCase().includes(searchQuery.toLowerCase())),
  ...
  const handlePrintComanda = async () => {
    ...
    const details = [
      settings.showClientData
        ? `<p><strong>Cliente:</strong> ${escapePrintHtml(receiptModalComanda.clientName)}</p>`
        : "",
      settings.showProfessional
        ? `<p><strong>Profissional:</strong> ${escapePrintHtml(receiptModalComanda.professionalName || "Geral")}</p>`
        : "",
      ...
      settings.showPayment
        ? `<p><strong>Pagamento:</strong> ${escapePrintHtml(receiptModalComanda.paymentMethod)}</p>`
        : "",
    ].join("");
    const items = settings.showService
      ? `<h2>Itens consumidos</h2>${receiptModalComanda.items.map((item) => `<div class="print-row"><span>${escapePrintHtml(`${item.quantity}x ${item.title}`)}</span><strong>${escapePrintHtml(`R$ ${(item.price * item.quantity).toFixed(2)}`)}</strong></div>`).join("")}`
      : "";
  ...
  return (
    <div className="space-y-4 animate-fade-in text-[var(--admin-text-main)] min-w-0">
      {/* Toast Notification */}
      {toastMsg && (
        <div className="fixed top-4 right-4 z-50 bg-[var(--admin-accent)] text-[var(--admin-accent-text)] px-4 py-2.5 rounded-xl font-bold text-xs shadow-xl flex items-center gap-2 animate-bounce">
      ...
      <button
        ...
        className="md:hidden w-full bg-[var(--admin-accent)] hover:bg-gold-hover text-[var(--admin-accent-text)] px-4 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 shrink-0"
      >
        ...
        tabs={[
          {
            id: "open",
            label: "Abertas",
            count: comandas.filter((c) => c.status === "open").length,
          },
          {
            id: "closed",
            label: "Fechadas",
            count: comandas.filter((c) => c.status === "closed").length,
          },
      ...
      {activeTab !== "new" && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl pl-10 pr-4 py-2.5 text-xs text-[var(--admin-text-main)] focus:outline-none focus:ring-1 focus:ring-[var(--admin-accent)]/50 placeholder:text-[var(--admin-text-muted)]/60"
          />
        </div>
      )}
      ...
      {activeTab === "open" && (
        <div>
          {openComandas.length === 0 ? (
            <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-2xl p-8 text-center space-y-3">
              <div className="w-12 h-12 rounded-full bg-[var(--admin-bg)] border border-[var(--admin-border)] text-[var(--admin-text-muted)] flex items-center justify-center mx-auto">
              ...
              <p className="text-xs text-[var(--admin-text-muted)] max-w-sm mx-auto">
                Sem comandas abertas. Crie uma para registrar atendimento ou
                consumo.
              </p>
              <button
                onClick={() => setActiveTab("new")}
                className="bg-[var(--admin-accent)] text-[var(--admin-accent-text)] px-4 py-2 rounded-xl text-xs font-bold inline-flex items-center gap-2 hover:bg-gold-hover transition-all"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {openComandas.map((cmd) => (
                <div
                  key={cmd.id}
                  className="bg-[var(--admin-surface)] border border-[var(--admin-border)] hover:border-[var(--admin-accent)]/40 rounded-2xl p-4 shadow-xs transition-all flex flex-col justify-between space-y-3 relative overflow-hidden group"
                >
                  <div className="absolute top-0 right-0 w-2 h-full bg-[var(--admin-accent)]" />

                  {/* Comanda Header */}
                  <div>
                    <div className="flex items-center justify-between border-b border-[var(--admin-border)]/60 pb-2 mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-mono font-bold text-[var(--admin-accent)] bg-[var(--admin-accent)]/10 px-2 py-0.5 rounded-xl">
                          {cmd.code}
                        </span>
                        <span className="text-xs text-[var(--admin-text-muted)] flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          {new Date(cmd.createdAt).toLocaleTimeString("pt-BR", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
                    ...
                    <div className="space-y-1 mb-3">
                      <div className="flex items-center gap-2 text-sm font-bold text-[var(--admin-text-main)]">
                        <User className="w-4 h-4 text-[var(--admin-accent)] shrink-0" />
                      ...
                      {cmd.professionalName && (
                        <div className="flex items-center gap-2 text-xs text-[var(--admin-text-muted)]">
                          <Scissors className="w-3.5 h-3.5 text-[var(--admin-text-muted)] shrink-0" />
                          <span>
                            Profissional:{" "}
                    ...
                    {/* Items List */}
                    <div className="bg-[var(--admin-bg)] rounded-xl p-2.5 border border-[var(--admin-border)]/80 space-y-1.5 max-h-36 overflow-y-auto custom-scrollbar">
                      <span className="text-xs font-bold uppercase text-[var(--admin-text-muted)] tracking-wider block mb-1">
                        Consumo na Comanda
                      </span>
                      {cmd.items.map((item, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-1.5 min-w-0 pr-2">
                            ...
                            <span className="text-[var(--admin-text-main)] truncate">
                              {item.title}
                            </span>
                          </div>
                  ...
                  {/* Total & Action */}
                  <div className="border-t border-[var(--admin-border)]/80 pt-3 flex items-center justify-between">
                    <div>
                    ...
                    <button
                      ...
                      className="bg-[var(--admin-accent)] hover:bg-gold-hover text-[var(--admin-accent-text)] px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md active:scale-95 transition-all"
                    >
      ...
      {activeTab === "closed" && (
        <div className="space-y-3">
          {closedComandas.length === 0 ? (
            <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-2xl p-8 text-center text-[var(--admin-text-muted)] text-xs">
      ...
      {activeTab === "new" && (
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl px-3 py-2 text-xs text-[var(--admin-text-main)] focus:outline-none focus:ring-1 focus:ring-[var(--admin-accent)]"
              />
            </div>
          ...
          <div className="space-y-3 bg-[var(--admin-bg)] p-4 rounded-xl border border-[var(--admin-border)]">
            <span className="text-xs font-bold text-[var(--admin-text-main)] uppercase tracking-wider block">
              Adicionar Consumo
            </span>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0">
                <select
                  ...
                  className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl px-3 py-2 text-xs text-[var(--admin-text-main)] focus:outline-none"
                >
              ...
              <button
                ...
                className="bg-[var(--admin-accent)] text-[var(--admin-accent-text)] px-3 py-2 rounded-xl text-xs font-bold hover:bg-gold-hover transition-colors shrink-0"
              >
                Adicionar
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  {products.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.name} — R$ {p.price.toFixed(2)} (Estoque:{" "}
                      {p.stock_quantity})
                    </option>
                  ))}
                </select>
            ...
            {newCartItems.length > 0 && (
              <div className="pt-2 border-t border-[var(--admin-border)]/80 space-y-2">
                <span className="text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider block">
                  Itens na Comanda ({newCartItems.length})
                </span>
                {newCartItems.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between bg-[var(--admin-surface)] p-2.5 rounded-xl border border-[var(--admin-border)]/60 text-xs"
                  >
                    <div>
                      <span className="font-bold text-[var(--admin-text-main)]">
                        {item.quantity}x {item.title}
                      </span>
                      <span className="text-xs text-[var(--admin-text-muted)] block uppercase">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-bold text-[var(--admin-text-muted)] hover:text-[var(--admin-text-main)] transition-colors"
            >
              Cancelar
            </button>
          ...
          <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-2xl w-full max-w-md p-5 text-[var(--admin-text-main)] space-y-4 relative shadow-2xl animate-fade-in">
            ...
            <div className="space-y-3 bg-[var(--admin-bg)] p-3.5 rounded-xl border border-[var(--admin-border)] text-xs">
              <div className="flex justify-between items-center text-[var(--admin-text-muted)]">
                ...
                <span className="font-bold finance-positive tabular-nums">
                  R$ {closingComanda.subtotal.toFixed(2)}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-sm font-bold border-t border-[var(--admin-border)]/80 pt-2 finance-positive">
                ...
                <span className="text-base tabular-nums">
                  ...
                  {Math.max(
                    0,
                    closingComanda.subtotal - discountInput + tipInput,
                  ).toFixed(2)}
                </span>
              </div>
            </div>
