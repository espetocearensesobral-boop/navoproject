...
export const ReceiptsManagement: React.FC = () => {
 ...
 <td>
 {isPending ? (
 <span className="inline-flex items-center gap-1 text-[11px] text-amber-500 bg-status-warning/10 px-2 py-0.5 rounded-[var(--admin-radius-sm)] border border-status-warning/30 font-medium">
 ...
 <span className="text-[10px] bg-status-success/10 text-emerald-500 px-1 py-0.2 rounded font-mono">
 -desc
 </span>
 )}
 {item.surchargeAmount > 0 && (
 <span className="text-[10px] bg-status-warning/10 text-amber-500 px-1 py-0.2 rounded font-mono">
 +acrés
 </span>
 )}
 </div>
 ...
 isPending
 ?"bg-status-warning/10 text-amber-500 border-status-warning/30"
 ...
 <div className="w-6 h-6 rounded-[var(--admin-radius-sm)] bg-status-warning/10 text-amber-500 flex items-center justify-center text-xs font-bold">
...
const StatusBadge: React.FC<{ status: ReceiptItem["status"] }> = ({ status }) => {
 ...
 :"bg-status-warning/10 text-amber-500 border border-status-warning/30"
 }`}
 >
