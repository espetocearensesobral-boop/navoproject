                  ...
                  {isExpanded && (
                    <div className="border-t border-border-subtle bg-surface-base/35 p-3.5 sm:p-4 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {client.phone && <a href={`https://wa.me/55${client.phone.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" title="Abrir WhatsApp" aria-label="Abrir WhatsApp" className="min-h-10 min-w-10 px-2 sm:px-4 rounded-xl border border-status-success/30 text-status-success text-sm font-semibold flex items-center justify-center gap-1.5"><Phone className="w-4 h-4" /><span className="hidden sm:inline">WhatsApp</span></a>}
                        <button type="button" onClick={() => handleOpenModal(client)} title="Editar cliente" aria-label="Editar cliente" className="min-h-10 min-w-10 px-2 sm:px-4 rounded-xl bg-gold-base text-surface-base text-sm font-bold flex items-center justify-center gap-1.5"><Edit2 className="w-4 h-4" /><span className="hidden sm:inline">Editar</span></button>
                        <button type="button" onClick={() => handleDelete(client.id)} title="Excluir cliente" aria-label="Excluir cliente" className="min-h-10 min-w-10 px-2 sm:px-4 rounded-xl border border-status-error/25 text-status-error text-sm font-semibold flex items-center justify-center gap-1.5"><Trash2 className="w-4 h-4" /><span className="hidden sm:inline">Excluir</span></button>
                        {isAdmin && <span title="Administrador" aria-label="Administrador" className="min-h-10 min-w-10 px-2 sm:px-4 rounded-xl bg-purple-500/15 text-purple-300 text-sm font-semibold flex items-center justify-center gap-1.5"><ShieldCheck className="w-4 h-4" /><span className="hidden sm:inline">Administrador</span></span>}
                      </div>
                    </div>
                  )}
                </article>
