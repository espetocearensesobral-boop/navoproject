...
const EmailSettingsTab: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="text-xs text-[var(--admin-text-muted)] py-8 text-center">
        Carregando e-mail...
      </div>
    );
  }
    ...
    <div className="space-y-6 w-full text-xs min-w-0">
      <div>
        ...
        <p className="text-xs text-[var(--admin-text-muted)] mb-4">
          Configure um servidor SMTP para enviar avisos à barbearia e aos
          clientes que informarem e-mail no agendamento. Você pode usar um
          provedor gratuito, como o Gmail (com uma "Senha de app"), Brevo ou
          Zoho Mail.
        </p>
      </div>
      ...
      {statusMsg && (
        <div
          className={`p-3 rounded-xl flex items-center gap-2 text-xs font-bold ${statusMsg.type === "success" ? "bg-status-success/10 border border-status-success/30 text-status-success" : "bg-status-error/10 border border-status-error/30 text-status-error"}`}
        >
      ...
      <div className="flex items-center justify-between bg-[var(--admin-bg)] p-3.5 rounded-xl border border-[var(--admin-border)]">
        <div>
          <p className="text-xs font-bold text-[var(--admin-text-main)]">
            Ativar envio de e-mails
          </p>
          <p className="text-xs text-[var(--admin-text-muted)]">
            Quando desativado, nenhum e-mail é enviado, mesmo com os dados
            preenchidos.
          </p>
        </div>
        <button
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-2.5 text-xs text-[var(--admin-text-main)] focus:outline-none focus:border-[var(--admin-accent)] min-w-0 num-tabular"
            />
          </div>
        </div>
        ...
        <label className="flex items-center gap-2 text-xs text-[var(--admin-text-muted)]">
          ...
          />
          Usar TLS implícito (marque para porta 465; deixe desmarcado para
          587/STARTTLS)
        </label>
        ...
        <div>
          <label className="text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider block mb-1">
            Senha SMTP{" "}
            {settings.hasPassword && !smtpPasswordInput && (
              <span className="normal-case font-normal text-[var(--admin-text-muted)]">
          ...
          <div className="relative">
            <input
              ...
              placeholder={
                settings.hasPassword ? "••••••••" : "Senha ou senha de app"
              }
              value={smtpPasswordInput}
              onChange={(e) => setSmtpPasswordInput(e.target.value)}
              className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-2.5 pr-9 text-xs text-[var(--admin-text-main)] focus:outline-none focus:border-[var(--admin-accent)] min-w-0"
            />
            <button
              type="button"
            ...
            fetchEmailSettings()
              ...
              .catch((e) =>
                setStatusMsg({
                  ...
                  text:
                    e.message ||
                    "Não foi possível restaurar as configurações salvas.",
                }),
              );
          }}
          disabled={isSaving}
          className="h-11 sm:h-10 w-full sm:w-auto px-5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)] text-[var(--admin-text-muted)] hover:text-[var(--admin-text-main)] hover:bg-[var(--admin-bg)] text-xs font-bold transition-colors disabled:opacity-50"
        >
          Cancelar
        </button>
        <button
      ...
      <div className="pt-4 border-t border-[var(--admin-border)] space-y-2">
        ...
        <p className="text-xs text-[var(--admin-text-muted)]">
          Salve os dados acima antes de testar. O teste funciona mesmo com o
          envio desativado.
        </p>
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-2.5 text-xs text-[var(--admin-text-main)] focus:outline-none focus:border-[var(--admin-accent)] min-w-0"
          />
          <button
            onClick={handleTestSend}
            disabled={isTesting || !testEmail.trim()}
            className="h-10 px-4 bg-[var(--admin-bg)] border border-[var(--admin-border)] hover:border-[var(--admin-accent)] text-[var(--admin-text-main)] rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 whitespace-nowrap shrink-0"
          >
...
const PrintSettingsTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchPrintSettings()
      .then((data) => {
        if (!cancelled) setSettings({ ...defaultPrintSettings, ...data });
      })
      .catch((error) => {
        if (!cancelled)
          setStatus({
    ...
    const format =
      kind === "receipt"
        ...
        : kind === "report"
          ...
          : settings.qrFormat;
    const width =
      format === "thermal" ? `${settings.thermalWidthMm}mm` : "210mm";
    const pageSize =
      format === "thermal"
        ? `${settings.thermalWidthMm}mm auto`
        : `A4 ${settings.a4Orientation}`;
    const density =
  ...
  const toggles: Array<[keyof PrintSettings, string, string]> = [
    ...
    [
      "showProfessional",
      "Exibir profissional",
      "Nome do barbeiro no documento.",
    ],
    [
      "showService",
      "Exibir serviço",
      "Serviço, item ou atendimento realizado.",
    ],
    ["showPayment", "Exibir pagamento", "Forma de pagamento e valores."],
    [
      "showObservations",
      "Exibir observações",
      "Observações digitadas no checkout.",
    ],
    [
      "showQr",
      "Exibir QR Code quando aplicável",
      "Inclui QR em documentos que suportam essa área.",
    ],
    ...
    <div className="space-y-6 w-full text-xs min-w-0">
      <div>
        ...
        <p className="text-xs text-[var(--admin-text-muted)] mt-1">
          Defina como comprovantes, relatórios e QR Codes devem ser preparados
          para impressora térmica ou folha A4. As mudanças afetam novos previews
          e impressões do navegador.
        </p>
      </div>

      {status && (
        <div
          className={`p-3 rounded-xl flex items-center gap-2 font-bold ${status.type === "success" ? "bg-status-success/10 border border-status-success/30 text-status-success" : "bg-status-error/10 border border-status-error/30 text-status-error"}`}
        >
      ...
      <section className="space-y-3">
        <h3 className="text-xs font-bold text-[var(--admin-text-main)] uppercase tracking-wider flex items-center gap-2">
          <Printer className="w-4 h-4 text-[var(--admin-accent)]" /> Modelo por
          documento
        </h3>
        <div className="admin-card-grid admin-card-grid--3">
          {(
            [
              ["receiptFormat", "Comprovantes", "receipt"],
              ["reportFormat", "Relatórios", "report"],
              ["qrFormat", "QR Code / Totem", "qr"],
            ] as const
          ).map(([key, label, kind]) => (
            <div
              key={key}
              className="bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl p-3 space-y-3"
            >
              <div>
              ...
              <select
                ...
                className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-2.5 text-[var(--admin-text-main)]"
              >
              ...
              <button
                ...
                className="w-full h-9 rounded-xl border border-[var(--admin-border)] text-[var(--admin-text-main)] font-bold flex items-center justify-center gap-2 hover:border-[var(--admin-accent)]"
              >
      ...
      <section className="bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl p-4 space-y-4">
        <div>
          <h3 className="text-xs font-bold text-[var(--admin-text-main)] uppercase tracking-wider">
            Papel e legibilidade
          </h3>
          <p className="text-xs text-[var(--admin-text-muted)] mt-1">
            Esses valores controlam largura, orientação, margem, tamanho e
            espaçamento no documento gerado.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <label className="space-y-1">
            ...
            <input
              ...
              className="w-full bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-2.5 text-[var(--admin-text-main)] num-tabular"
            />
          </label>
          <label className="space-y-1">
            <span className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase">
        ...
        <label className="space-y-1 block max-w-sm">
          ...
          >
            ...
            <option value="spacious">Espaçosa — mais respiro</option>
          </select>
        </label>
      </section>

      <section className="bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl p-4 space-y-3">
        <div>
          <h3 className="text-xs font-bold text-[var(--admin-text-main)] uppercase tracking-wider">
            Detalhes dos comprovantes
          </h3>
          <p className="text-xs text-[var(--admin-text-muted)] mt-1">
            Controle o que aparece no recibo sem apagar dados do banco.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {toggles.map(([key, label, description]) => (
            <label
              key={String(key)}
              className="flex items-start gap-2 p-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)]"
            >
              <input
                type="checkbox"
                checked={Boolean(settings[key])}
                onChange={(e) =>
                  update(key, e.target.checked as PrintSettings[typeof key])
                }
                className="mt-0.5 accent-gold-base"
              />
              <span>
      ...
      <section className="bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl p-4 space-y-3">
        <div>
          ...
          <p className="text-xs text-[var(--admin-text-muted)] mt-1">
            O CSV continua independente; estas opções controlam a impressão
            visual do relatório.
          </p>
        </div>
        <label className="flex items-start gap-2 p-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)]">
          <input
            type="checkbox"
            checked={settings.reportIncludeCharts}
            onChange={(e) => update("reportIncludeCharts", e.target.checked)}
            className="mt-0.5 accent-gold-base"
          />
          <span>
            <strong className="text-[var(--admin-text-main)]">
              Incluir gráficos e indicadores
            </strong>
            <span className="block text-xs text-[var(--admin-text-muted)] mt-0.5">
              Inclui a leitura visual do período quando houver dados.
            </span>
          </span>
        </label>
        <label className="flex items-start gap-2 p-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)]">
          <input
            ...
            onChange={(e) => update("reportIncludeDetails", e.target.checked)}
            className="mt-0.5 accent-gold-base"
          />
          <span>
            <strong className="text-[var(--admin-text-main)]">
              Incluir detalhes e rankings
            </strong>
            <span className="block text-xs text-[var(--admin-text-muted)] mt-0.5">
              Inclui serviços, formas de pagamento, despesas e clientes.
            </span>
          </span>
        </label>
      </section>

      <div className="pt-4 border-t border-[var(--admin-border)] flex flex-col sm:flex-row sm:justify-end gap-2">
        <button
          type="button"
          onClick={handleReset}
          disabled={saving}
          className="h-11 sm:h-10 w-full sm:w-auto px-5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)] text-[var(--admin-text-muted)] font-bold flex items-center justify-center gap-2"
        >
